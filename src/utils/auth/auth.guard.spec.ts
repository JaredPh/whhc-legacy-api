...
describe('AuthGuard', () => {
    ...
    describe('canActivate()', () => {
        ...
        let authServiceVerifyEncryptedKeyStub: sinonStub;
        let authServiceGetMemberStub: sinonStub;
        let mockReq: any;
        let result: boolean;

        describe('using an encrypted key for authentication', () => {
            beforeEach(() => {
                mockReq = {
                    query: {},
                    body: {},
                ...
                reflectorGetStub = sinon.stub(reflector, 'get')
                    .onFirstCall().returns('mockEventType');
            });

            afterEach(() => {
                reflectorGetStub.restore();
                authServiceVerifyEncryptedKeyStub.restore();
            });

            it('should return false if there is no key', async () => {
                authServiceVerifyEncryptedKeyStub = sinon.stub(authService, 'verifyEncryptedKey');
                result = await authGuard.canActivate(mockReq, mockContext);

                expect(authServiceVerifyEncryptedKeyStub).to.have.not.been.called;
            ...
            it('should return false if there is a key and the authService method verifyEncryptedKey returns false', async () => {
                ...
                expect(authServiceVerifyEncryptedKeyStub).to.have.been.called;
                expect(result).to.be.false;
            });
        ...
        describe('using a JWT for authentication', () => {
                ...
                describe('and a required role', () => {
                    ...
                    describe('where multiple roles matche', () => {
                        before(async () => {
                            ...
                            reflectorGetStub = sinon.stub(reflector, 'get')
                                .onFirstCall().returns(undefined)
                                .onSecondCall().returns(['member', 'role1', 'committee']);
                    ...
                    describe('where multiple roles don\'t match', () => {
                        before(async () => {
                            ...
                            reflectorGetStub = sinon.stub(reflector, 'get')
                                .onFirstCall().returns(undefined)
                                .onSecondCall().returns(['role1', 'role2', 'role3']);
